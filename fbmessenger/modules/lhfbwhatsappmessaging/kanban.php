...
$tpl = erLhcoreClassTemplate::getInstance('lhfbwhatsappmessaging/kanban.tpl.php');

if (isset($_POST['doDelete'])) {
    if (!isset($_POST['csfr_token']) || !$currentUser->validateCSFRToken($_POST['csfr_token'])) {
        erLhcoreClassModule::redirect('fbwhatsappmessaging/kanban');
        exit;
    }
...
if (isset($Params['user_parameters_unordered']['export']) && $Params['user_parameters_unordered']['export'] == 1) {
    if (ezcInputForm::hasPostData()) {
        ...
        $tpl->set('action_url', erLhcoreClassDesign::baseurl('fbwhatsappmessaging/kanban') . erLhcoreClassSearchHandler::getURLAppendFromInput($filterParams['input_form']));
        echo $tpl->fetch();
        exit;
    }
}
...
$kanbans = erLhcoreClassModelKanban::getList();
...
try {
    ...
    $pages->serverURL = erLhcoreClassDesign::baseurl('fbwhatsappmessaging/kanban') . $append;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['movements'])) {
    ...
    foreach ($movements as $movement) {
        ...
        $column_data = erLhcoreClassModelKanban::getList(['filter' => ['nombre' => $movement['column']]]);
        $kanban_id = 0;
...
$filterParams['input_form']->form_action = erLhcoreClassDesign::baseurl('fbwhatsappmessaging/kanban');
...
$Result['path'] = array(
    array(
        'url' => erLhcoreClassDesign::baseurl('fbwhatsappmessaging/kanban'),
